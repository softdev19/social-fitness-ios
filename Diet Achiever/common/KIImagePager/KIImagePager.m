...
- (void) loadData
{
    ...
    if([aImageUrls count] > 0) {
        ...
        if(_custom_turtorial){
            for (int i = 0; i < [aImageUrls count]; i++) {
                CGRect imageFrame = CGRectMake(_scrollView.frame.size.width * i + 5, 0 + 5, _scrollView.frame.size.width - 10, _scrollView.frame.size.height );
                ...
                [_scrollView addSubview:imageView];
                 ...
                 CGRect bottomFrame = CGRectMake(_scrollView.frame.size.width * i , _scrollView.frame.size.height - 30, _scrollView.frame.size.width,  30);
            ...
            for (int i = 0; i < [aImageUrls count]; i++) {
                ...
                /*custom for sportsherder*/
                if(self.custom_wang){
                    UIImageView *wrapperImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"team_back_grad.png"]];
                    wrapperImage.frame = imageView.frame;
                    [_scrollView addSubview:wrapperImage];
                    
                    UILabel *title;
                    if(isiPhone4)
                    {
                        title = [[UILabel alloc]initWithFrame:CGRectMake(_scrollView.frame.size.width * i + 10, 90, _scrollView.frame.size.width - 20,60)];
                        [title setFont:[UIFont fontWithName:@"RobotoCondensed" size:8.0]];
                    }
                    if(isiPhone5){
                        title = [[UILabel alloc]initWithFrame:CGRectMake(_scrollView.frame.size.width * i + 10, 100, _scrollView.frame.size.width - 20,60)];
                        [title setFont:[UIFont fontWithName:@"RobotoCondensed" size:8.0]];
                    }
                    if(isiPhone6){
                        title = [[UILabel alloc]initWithFrame:CGRectMake(_scrollView.frame.size.width * i + 10, 170, _scrollView.frame.size.width - 20,60)];
                        [title setFont:[UIFont fontWithName:@"RobotoCondensed" size:10.0]];
                    }
                    if(isiPhone6plus){
                        title = [[UILabel alloc]initWithFrame:CGRectMake(_scrollView.frame.size.width * i + 10, 180, _scrollView.frame.size.width - 20,60)];
                        [title setFont:[UIFont fontWithName:@"RobotoCondensed" size:10.0]];
                    }                    
        ...
        _pageControl.currentPageIndicatorTintColor = [UIColor colorWithRed:104/255.0 green:191/255.0 blue:222/255.0 alpha:1.0];
